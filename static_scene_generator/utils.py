...
def compute_camera_traj(p0, num_views):
  """
  Compute view points for multi-view traverses. Traverse resolution depends on
  the number of view points. Since the camera is set to track world origin,
  we need to compute only the locations.
  """
  if isinstance(p0, list):
      p0 = tuple(p0)
  assert isinstance(p0, tuple)
  x0, y0, z0 = p0
  mag_xy = math.sqrt(math.pow(x0, 2) + math.pow(y0, 2))
  theta_0 = math.atan2(y0, x0)

  thetas = np.arange(0, 1, 1.0/float(num_views),
                     dtype='float32')*math.pi*2.0 + theta_0
  xs = (mag_xy * np.cos(thetas)).tolist()
  ys = (mag_xy * np.sin(thetas)).tolist()
  zs = num_views * [z0]
